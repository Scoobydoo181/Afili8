...
test("Create link", async () => {
    try {
        await axios.post("http://localhost:" + port + "/api/delete", {clientToken: 'TEST', afiliateCode: 'abc'})
        
        await axios.post("http://localhost:" + port + "/api/create", {clientToken: 'TEST', afiliateCode: 'abc'})
    
        let result = await db.collection('clients.TEST').findOne({afiliateCode: 'abc'})    
        
        expect(result).toEqual(expect.objectContaining({afiliateCode: 'abc'}))
    } catch (error) {
...
test("Verify link", async () => {
    let res = await axios.post("http://localhost:" + port + "/api/verify", {clientToken: 'TEST', afiliateCode: 'abc'})
    expect(res.data).toBe(true);
});

test("Increment link", async () => {
    try {
        //await db.collection("clients.TEST").insertOne({ afiliateCode: "abc", numberUses: 0 });
    
        let result1 = await db.collection('clients.TEST').findOne({afiliateCode: 'abc'})
        result1 = result1.numberUses
        
        await axios.post("http://localhost:" + port + "/api/increment", {clientToken: 'TEST', afiliateCode:'abc'});
    
        let result2 = await db.collection('clients.TEST').findOne({afiliateCode: 'abc'})
